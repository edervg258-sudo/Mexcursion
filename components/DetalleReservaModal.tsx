...
function politicaCancelacion(dias: number): {
  ...
  if (dias > 30) {
    return { titulo: 'Cancelación gratuita', descripcion: 'Puedes cancelar sin costo hasta 30 días antes de tu viaje.', color: '#3AB7A5', icono: 'checkmark-circle' };
  }
  if (dias > 14) {
    return { titulo: 'Cancelación con 25% de cargo', descripcion: 'Entre 15 y 30 días antes del viaje aplica un cargo del 25% del total.', color: '#f5a623', icono: 'alert-circle' };
  }
  if (dias > 7) {
    return { titulo: 'Cancelación con 50% de cargo', descripcion: 'Entre 8 y 14 días antes aplica un cargo del 50% del total.', color: '#f5a623', icono: 'alert-circle' };
  }
  if (dias >= 0) {
    return { titulo: 'Sin reembolso', descripcion: 'A menos de 7 días del viaje no aplican reembolsos.', color: '#DD331D', icono: 'close-circle' };
  }
  return {
