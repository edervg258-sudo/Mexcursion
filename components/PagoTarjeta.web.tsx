...
export function PagoTarjeta({
  ...
  const handleCheckout = async () => {
    ...
    if (numeroNormalizado.length < 16) {
      const mensaje = 'Ingresa un número de tarjeta válido.';
      setError(mensaje);
      onError(mensaje);
      return;
    }
    if (!/^\d{2}\/\d{2}$/.test(vencimiento.trim())) {
      const mensaje = 'Ingresa una fecha de vencimiento válida en formato MM/AA.';
      setError(mensaje);
      onError(mensaje);
      return;
    }
    if (!/^\d{3,4}$/.test(cvv.trim())) {
      const mensaje = 'Ingresa un CVV válido.';
      setError(mensaje);
      onError(mensaje);
      return;
    }
    ...
    setTimeout(() => {
      setLoading(false);
      onSuccess(`web-sim-${externalReference}`);
    }, 900);
  };
    ...
    >
      <View style={estilos.header}>
        ...
        <Text style={[estilos.headerSubtitulo, { color: isDark ? '#aaa' : '#666' }]}>
          Pago simulado para pruebas en web
        </Text>
      </View>
      ...
      <View style={[estilos.formulario, { backgroundColor: isDark ? '#222' : '#fff', borderColor: isDark ? '#444' : '#ddd' }]}>
        ...
        <View style={estilos.formularioCuerpo}>
          ...
          <TextInput
            ...
            placeholderTextColor={isDark ? '#888' : '#999'}
            value={numero}
            onChangeText={(value) => { setNumero(value); if (error) { setError(null); } }}
          ...
          <View style={estilos.fila}>
            ...
            <TextInput
              ...
              placeholderTextColor={isDark ? '#888' : '#999'}
              value={cvv}
              onChangeText={(value) => { setCvv(value); if (error) { setError(null); } }}
              keyboardType="number-pad"
              maxLength={4}
            />
          </View>
          <View style={[estilos.infoBox, { backgroundColor: isDark ? '#2a3f3f' : '#f0faf9', borderColor: isDark ? '#3a5f5f' : '#d1e8e5' }]}>
            <Text style={[estilos.infoLabel, { color: isDark ? '#aaa' : '#666' }]}>
              Usa cualquier tarjeta de prueba para continuar.
            </Text>
          </View>
        </View>
      </View>
      ...
      <TouchableOpacity
        ...
        activeOpacity={0.85}
      >
        <Text style={estilos.btnVolverText}>Volver a métodos de pago</Text>
      </TouchableOpacity>
